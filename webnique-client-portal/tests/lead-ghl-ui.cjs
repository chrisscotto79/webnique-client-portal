...
(async () => {
  ...
  try {
    ...
    confirmed = false;
    page.once('dialog', async dialog => { confirmed = dialog.message().includes('live emails') && dialog.message().includes('Other holds stay untouched'); await dialog.dismiss(); });
    await page.click('input[value=retry_dnd] ~ p button');
    assert(confirmed, 'Targeted DND retry confirms live-email effect and scope');
