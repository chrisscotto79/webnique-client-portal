    ...
    try {
        ...
        await page.evaluate(() => {
            ...
            window.fetch = async (_, args) => {
                ...
                if (op === 'progress') data = {enabled, counts: {total: 1, submitted: 0, pending: 0, skipped: 0, review: done ? 1 : 0}, rows: [{url, status: done ? 'skipped' : 'waiting', message: ''}], review: []};
                if (op === 'next') data = done ? {waiting: true, next_at: Date.now()/1000 + 60, message: 'Waiting for the one-minute posting interval.'} : {job: {images: [], image_count: 0, client_id: window.badClient || 'agency', client_name: 'Golden Web Marketing', key: 'job', token: 'token', url, message: 'hello'}};
                return {ok: true, json: async () => ({success: true, data})};
            };
            window.addEventListener('message', event => {
                if (event.data.source !== 'wnq-facebook-page') return;
                if (event.data.op === 'publish') publishCalls++;
                const result = event.data.op === 'publish' ? {status: 'unknown', scope: 'group', message: 'Check Facebook; submission not confirmed.'} : {version: '1.2.0'};
        ...
        await page.waitForFunction(() => document.getElementById('fb-status').textContent.includes('one-minute'));
        assert(await page.$eval('#fb-errors', el => !el.hidden && el.textContent.includes('not confirmed')));
        assert(await page.$eval('[data-fb-group]', el => el.textContent.includes('Skipped')));
