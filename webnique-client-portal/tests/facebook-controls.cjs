...
(async () => {
    ...
    try {
        ...
        await page.evaluate(() => {
            ...
            window.operations = []; window.enabled = false; window.done = false;
            const nativeTimeout = setTimeout;
            window.setTimeout = (fn, ms) => nativeTimeout(fn, ms === 10000 ? 50 : ms);
            window.fetch = async (_, args) => {
                const op = args.body.get('op'); operations.push(op);
                ...
                if (op === 'next') data = done ? {waiting: true, next_at: Date.now()/1000 + 360, message: 'Waiting for the six-minute posting interval.'} : {job: {key: 'job', token: 'token', url, message: 'hello'}};
                return {ok: true, json: async () => ({success: true, data})};
            };
            window.addEventListener('message', event => {
                if (event.data.source !== 'wnq-facebook-page') return;
                const result = event.data.op === 'publish' ? {status: 'unknown', scope: 'group', message: 'Check Facebook; submission not confirmed.'} : {version: '1.0.7'};
