...
(async()=>{
    ...
    window.jQuery=$;`;
    const html='<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Client results QA</title><style>body{background:#f0f2f5;font:14px system-ui;padding:20px;margin:0}main{max-width:1200px;margin:auto}button,input,select{font:inherit;padding:6px;border:1px solid #bdc9d8;border-radius:4px;background:white}h1{color:#163756}@media(max-width:600px){body{padding:10px}}</style><style>'+css+'</style><main><h1>Client results · QA preview</h1><select id="wnq-date-range"><option value="7">Last 7 Days</option><option value="30">Last 30 Days</option></select><button id="wnq-refresh-data">Refresh</button><section id="wnq-results-activity"><header class="wnq-results-header"><span>CLIENT RESULTS</span><h2>Calls, phone clicks &amp; form arrivals</h2><p>Three evidence sources. Separate counts—not a combined unique-lead total.</p></header><div id="wnq-activity-feeds"></div></section></main><script>'+mock+'</script><script>'+script+'</script></html>';
