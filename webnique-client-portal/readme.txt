...
- Adds website-backed factual-claim verification plus client-domain source configuration; unsupported claims are marked for review, never declared false.
- Keeps all recommendations read-only and does not generate, edit, pause, or publish Google Ads copy.
...
- Keeps every diagnostic read-only, privately permissioned, and independently recoverable when a Google Ads report is unavailable.
...
- Composes the approved Elementor template in a fixed section order without AI rearranging or duplicating sections
- Preserves template styling, media, gallery, and review widgets while generating localized copy, FAQs, contact form fields, and CTA content
...
- Grounded answers in allowlisted WordPress client, billing, task, request, report, CRM, and Google Ads data
- Added a dedicated AI Knowledge Base for SOPs and internal reference information with secure text and DOCX extraction
- Added client-name matching, clarification prompts, factual fallbacks, source labels, rate-limit protection, and an AI answer audit log
- Kept credentials out of AI context and retained explicit task commands as the only Telegram write actions
- Reduced Telegram question polling to one-minute intervals with overlap locks and per-poll limits
