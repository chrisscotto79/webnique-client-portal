...
final class LeadFinderAdmin
{
    ...
    private static function renderSearchTab(array $settings): void
    {
        ...
        <div class="wnq-card">
            ...
            <p style="color:#6b7280;margin:-6px 0 16px;font-size:12px;">
                Searches Google Places, filters out franchises and duplicates, crawls each website for SEO issues, extracts owner name + social media, then saves qualified leads.
                Each run may take <strong>5–15 minutes</strong> depending on volume — that's expected.
            </p>
            ...
            <div class="wnq-row2">
                ...
                <div class="wnq-field">
                    <label>City</label>
                    <input type="text" id="lf-city" placeholder="e.g. Orlando FL, Baltimore MD, Charlotte NC">
                </div>
            </div>
            <div class="wnq-row4">
                <div class="wnq-field">
                ...
                <div class="wnq-field">
                    ...
                    <small>0–7 issues. 2+ = has real SEO gaps.</small>
                </div>
                <div class="wnq-field">
                    ...
                    <select id="lf-max-results">
                        ...
                        <option value="60" selected>60 (max per run)</option>
                    </select>
                </div>
            </div>

            <button class="wnq-btn wnq-btn-primary" id="lf-btn" onclick="wnqSearch()">
                Search &amp; Qualify
            </button>

            <div class="wnq-progress" id="lf-progress">
                <div class="wnq-spinner"></div>
                <span>Running pipeline: Google Places &rarr; franchise filter &rarr; SEO scoring &rarr; owner &amp; social enrichment&hellip;
                This can take <strong>5–15 minutes</strong>. Please keep this tab open.</span>
            </div>
        ...
        function wnqSearch() {
            ...
            document.getElementById('lf-btn').disabled = true;
            document.getElementById('lf-progress').classList.add('show');
            document.getElementById('lf-result').innerHTML = '';

            const fd = new FormData();
            fd.append('action',      'wnq_lead_search');
            fd.append('nonce',       '<?php echo esc_js($nonce); ?>');
            fd.append('keyword',     kw);
            fd.append('city',        city);
            fd.append('min_reviews', document.getElementById('lf-min-reviews').value);
            fd.append('min_rating',  document.getElementById('lf-min-rating').value);
            fd.append('min_seo',     document.getElementById('lf-min-seo').value);
            fd.append('max_results', document.getElementById('lf-max-results').value);
            ...
            fetch(ajaxurl, { method:'POST', body: fd })
                ...
                .then(resp => {
                    ...
                    if (resp.success) {
                        ...
                        el.innerHTML = `<div class="wnq-result wnq-result-ok">
                            <strong>Search complete</strong>
                            <ul>
                                <li>Places found: <b>${s.found}</b></li>
                                <li>Franchises skipped: <b>${s.franchise_skipped}</b></li>
                                <li>Passed review/rating filter: <b>${s.filtered}</b></li>
                                <li>Had a website: <b>${s.filtered - s.no_website}</b></li>
                                <li>Passed SEO threshold: <b>${s.qualified}</b></li>
                                <li>Already in database: <b>${s.skipped_existing}</b></li>
                                <li><b>New leads saved: ${s.saved}</b></li>
                            </ul>
                            ${s.saved > 0 ? '<br><a href="admin.php?page=wnq-lead-finder&tab=leads" style="color:#15803d;font-weight:600">View saved leads &rarr;</a>' : ''}
                        </div>`;
                    } else {
                        el.innerHTML = `<div class="wnq-result wnq-result-err">
                ...
                .catch(err => {
                    ...
                    document.getElementById('lf-btn').disabled = false;
                    document.getElementById('lf-result').innerHTML =
                        `<div class="wnq-result wnq-result-err"><strong>Request failed</strong><p>${err.message}</p></div>`;
                });
        }
        </script>
        <?php
