  ...
  window.addEventListener('message', event => {
    ...
    try { chrome.runtime.sendMessage({action:msg.action, payload:msg.payload || {}}, response => {
      ...
      window.postMessage({channel:'wnq-leads-response',id:msg.id,
        response:error ? {ok:false,retryable:!String(error.message||'').includes('context invalidated'),error:'Chrome closed the companion message channel. Checking the saved search; if the extension was reloaded, refresh this WordPress page once.'} : (response || {ok:false,retryable:true,error:'Companion returned no result. Checking the saved search.'})}, location.origin);
    }); } catch (_) {
