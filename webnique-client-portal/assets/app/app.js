(function () {
    ...
    view.innerHTML = `${heading("Internal Reporting", "Google Ads", "Read-only performance from the Google Ads accounts linked to your manager account.")}
      ...
      <div class="wnq-ads-sections">
        <section class="wnq-ads-section ${state.adsSection === "overview" ? "is-active" : ""}" data-ads-panel="overview">
          ${resource.configured ? `<section class="wnq-metrics wnq-ads-metrics">${metric("Clicks", Number(summary.clicks || 0).toLocaleString(), resource.reporting_window || "This month")}${metric("Impressions", Number(summary.impressions || 0).toLocaleString(), resource.reporting_window || "This month")}${metric("CTR", percent(summary.ctr), "Click-through rate")}${metric("Conversions", Number(summary.conversions || 0).toFixed(1), "Tracked actions")}${metric("Conversion Rate", percent(summary.conversion_rate), "Conversions per click")}${metric("Campaigns", rows.length.toLocaleString(), `${enabledCampaigns} enabled`)}</section>
            ...
            <div class="wnq-ads-overview-grid">${adsTable("Campaign Snapshot", campaignColumns, rows.slice(0, 6), campaignRow, "No campaign performance was returned this month.")}${adsTable("Device Performance", ["Device", "Clicks", "Impressions", "Conversions"], resource.devices || [], (row) => [esc(titleCase(row.device)), row.clicks, row.impressions, Number(row.conversions || 0).toFixed(1)], "No device data was returned.")}</div>` : `<section class="wnq-panel wnq-ads-empty-report"><div><span class="wnq-eyebrow">Setup required</span><h2>Connect an Ads account to begin</h2><p>Open Connection to review the account link and credentials checklist.</p></div><button type="button" class="wnq-button" data-open-ads-connection>Open Connection</button></section>`}
        </section>
        <section class="wnq-ads-section ${state.adsSection === "campaigns" ? "is-active" : ""}" data-ads-panel="campaigns">${adsTable("Campaign Performance", campaignColumns, rows, campaignRow, "No campaign performance was returned this month.")}</section>
  ...
  const adsSettingsForm = (data) => {
    ...
    <label class="wnq-money-field"><span>Monthly Spend Alert</span><div><b>$</b><input type="number" name="spend_alert_threshold" value="${esc(Number(data.spend_alert_threshold || 0).toFixed(2))}" min="0" step="0.01"></div><small>Telegram alerts when this client reaches this amount during the current month. Use $0 to disable the alert.</small></label>
