(function () {
  ...
  const shell = () => {
    root.innerHTML = `<div class="wnq-portal">
      ...
      <main class="wnq-main"><div class="wnq-topbar"><div><span>Golden Web Marketing <em>v${esc(cfg.version || "unknown")}</em></span><strong id="wnq-top-title">Overview</strong></div><div class="wnq-topbar-actions"><button type="button" class="wnq-button is-secondary" id="wnq-fullscreen-toggle" title="Open the portal in full screen">Full Screen</button><button type="button" class="wnq-button is-secondary" id="wnq-refresh-view">Refresh</button></div></div><div id="wnq-view" role="main" aria-busy="true">${empty("Loading dashboard...")}</div></main></div>`;
  ...
  async function overview(view, refresh) {
    ...
    view.innerHTML = `${heading("Dashboard", `Welcome back, ${data.client?.company || data.client?.name || "there"}`, "Here is what is happening and what needs attention.")}
      ...
      <section class="wnq-grid-2">
        <div class="wnq-panel"><div class="wnq-panel-head"><h2>Action Items</h2></div>${actions.length ? actions.map((item) => `<button type="button" class="wnq-action" data-go="${esc(item.type)}"><strong>${esc(item.label)}</strong><span>Review</span></button>`).join("") : empty("Nothing needs your attention.")}</div>
        <div class="wnq-panel"><div class="wnq-panel-head"><h2>Latest Report</h2></div>${data.latest_report ? `<strong>${esc(reportTitle(data.latest_report))}</strong><p>${reportMonth(data.latest_report.period_start, data.latest_report.period_end)}</p><button type="button" class="wnq-button" data-go="reports">View reports</button>` : empty("Your first report will appear here.")}</div>
      </section>`;
  ...
  async function customers(view, refresh, options = {}) {
    ...
    view.querySelectorAll("[data-followup]").forEach((button) => button.addEventListener("click", async () => {
      ...
      const action = button.dataset.followupAction || "reschedule";
      const nextDate = action === "complete" ? "" : window.prompt("Next follow-up date (YYYY-MM-DD).", row.follow_up_date || "");
      if (nextDate === null) return;
      button.disabled = true;
      try {
        await api("/portal/customers", { method: "POST", body: JSON.stringify({ ...recordPayload(row), follow_up_date: nextDate }) });
        sessionStorage.setItem("wnqCrmNotice", nextDate ? "Follow-up rescheduled." : "Follow-up completed.");
    ...
    view.innerHTML = `${heading("Internal Reporting", "Google Ads", "Read-only performance from the Google Ads accounts linked to your manager account.")}
      ...
      <div class="wnq-ads-sections">
        <section class="wnq-ads-section ${state.adsSection === "overview" ? "is-active" : ""}" data-ads-panel="overview">
          ${resource.configured ? `<section class="wnq-metrics wnq-ads-metrics">${metric("Clicks", Number(summary.clicks || 0).toLocaleString(), resource.reporting_window || "Past 31 days")}${metric("Impressions", Number(summary.impressions || 0).toLocaleString(), resource.reporting_window || "Past 31 days")}${metric("CTR", percent(summary.ctr), "Click-through rate")}${metric("Conversions", Number(summary.conversions || 0).toFixed(1), "Tracked actions")}${metric("Conversion Rate", percent(summary.conversion_rate), "Conversions per click")}${metric("Campaigns", rows.length.toLocaleString(), `${enabledCampaigns} enabled`)}</section>
            ...
            ${adsBillingCard(resource.billing || {})}
            ${!resource.has_report_data ? adsEmptyReport(resource) : ""}
            <div class="wnq-ads-overview-grid">${adsTable("Campaign Snapshot", campaignColumns, rows.slice(0, 6), campaignRow, "No campaign performance was returned for the past 31 days.")}${adsTable("Device Performance", ["Device", "Clicks", "Impressions", "Conversions"], resource.devices || [], (row) => [esc(titleCase(row.device)), row.clicks, row.impressions, Number(row.conversions || 0).toFixed(1)], "No device data was returned.")}</div>` : `<section class="wnq-panel wnq-ads-empty-report"><div><span class="wnq-eyebrow">Setup required</span><h2>Connect an Ads account to begin</h2><p>Open Connection to review the account link and credentials checklist.</p></div><button type="button" class="wnq-button" data-open-ads-connection>Open Connection</button></section>`}
        </section>
        <section class="wnq-ads-section ${state.adsSection === "campaigns" ? "is-active" : ""}" data-ads-panel="campaigns">${adsTable("Campaign Performance", campaignColumns, rows, campaignRow, "No campaign performance was returned for the past 31 days.")}</section>
  ...
  const adsBillingCard = (billing = {}) => `<section class="wnq-panel wnq-ads-billing-card"><div class="wnq-panel-head"><div><span class="wnq-eyebrow">Admin only</span><h2>Billing Account</h2><small>Account setup from Google Ads API</small></div>${status(billing.available ? "green" : "yellow", billing.available ? titleCase(billing.status || "connected") : "Unavailable")}</div><div class="wnq-ads-billing-grid"><div><span>Payments account</span><strong>${esc(billing.payments_account_name || "Available in Google Ads")}</strong><small>${esc(billing.payments_account_id || "Payment activity is not exposed by the API")}</small></div><div><span>Payments profile</span><strong>${esc(billing.payments_profile_name || "Managed in Google Ads")}</strong><small>${esc(billing.payments_profile_id || "")}</small></div><div><span>Payment activity</span><strong>Managed by Golden Web Marketing</strong><small>Clients cannot open Google Ads Billing from the portal. Credit, payment method, and payment history remain private.</small></div></div></section>`;
  ...
  const adsSettingsForm = (data) => {
    ...
    const accountOptions = `<option value="">Auto-match by client name</option>${data.customer_id && !currentExists ? `<option value="${esc(data.customer_id)}" selected>${esc(data.matched_account_name || data.customer_id)}</option>` : ""}${accounts.map((account) => `<option value="${esc(account.customer_id)}" ${String(account.customer_id) === String(data.customer_id) ? "selected" : ""}>${esc(account.name || "Unnamed account")} · ${esc(account.customer_id)}</option>`).join("")}`;
    const accountControl = data.customer_id
      ? `<div class="is-wide wnq-ads-linked-account"><span>Linked client account</span><strong>${esc(data.matched_account_name || "Google Ads account")}</strong><small>Customer ID ${esc(data.customer_id)} · This portal is locked to this account.</small></div><details class="is-wide wnq-ads-account-change"><summary>Change linked account</summary><label><span>Administrator account mapping</span><select name="customer_id">${accountOptions}</select><small>Changing this affects only the selected portal client. This list is never sent to client users.</small></label></details>`
      : `<label><span>Google Ads Client Account</span><select name="customer_id">${accountOptions}</select><small>Select the one Ads account that belongs to this portal client.</small></label>`;
    return `<form class="wnq-panel wnq-form wnq-ads-link-form" id="wnq-ads-settings"><div class="wnq-panel-head"><div><span class="wnq-eyebrow">Account Link</span><h2>Client Ads Account</h2><small>Global OAuth credentials are managed in the WordPress portal settings.</small></div>${status("green", "Admin only")}</div>
    ...
    <label class="wnq-money-field"><span>31-Day Spend Alert</span><div><b>$</b><input type="number" name="spend_alert_threshold" value="${esc(Number(data.spend_alert_threshold || 0).toFixed(2))}" min="0" step="0.01"></div><small>Telegram alerts when this client reaches this amount during the past 31 days. Use $0 to disable the alert.</small></label>
  ...
  async function messages(view, refresh) {
    ...
    view.innerHTML = `${heading("Support", "Support Tickets", "Create a request, track its status, and keep every reply together.")}
      <div class="wnq-toolbar wnq-ticket-toolbar"><input type="search" id="wnq-ticket-search" aria-label="Search support tickets" placeholder="Search tickets"><select id="wnq-ticket-filter" aria-label="Filter support tickets"><option value="all">All tickets</option><option value="open">Open</option><option value="in_progress">In progress</option><option value="waiting">Waiting</option><option value="resolved">Resolved</option><option value="closed">Closed</option></select><button type="button" class="wnq-button" id="wnq-new-ticket">New Support Ticket</button></div><div id="wnq-ticket-compose"></div>
