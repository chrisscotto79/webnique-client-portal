(function () {
  ...
  const adsBillingCard = (billing = {}) => `<section class="wnq-panel wnq-ads-billing-card"><div class="wnq-panel-head"><div><span class="wnq-eyebrow">Admin only</span><h2>Billing Account</h2><small>Account setup from Google Ads API</small></div>${status(billing.available ? "green" : "yellow", billing.available ? titleCase(billing.status || "connected") : "Open in Google Ads")}</div><div class="wnq-ads-billing-grid"><div><span>Payments account</span><strong>${esc(billing.payments_account_name || "Available in Google Ads")}</strong><small>${esc(billing.payments_account_id || "Payment activity is not exposed by the API")}</small></div><div><span>Payments profile</span><strong>${esc(billing.payments_profile_name || "Managed in Google Ads")}</strong><small>${esc(billing.payments_profile_id || "")}</small></div><div><span>Payment activity</span><strong>Google Ads only</strong><small>Credit, payment method, and last or next payment stay in Google Ads.</small></div></div><a class="wnq-button is-secondary" href="https://ads.google.com/aw/billing/summary" target="_blank" rel="noopener">Open Google Ads Billing</a></section>`;
